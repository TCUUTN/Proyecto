...
function SolicitudesCarta() {
  ...
  const [searchEstudiantePendientes, setSearchEstudiantePendientes] = useState("");
  const [searchSocioCompletadas, setSearchSocioCompletadas] = useState("");
  const [searchEstudianteCompletadas, setSearchEstudianteCompletadas] = useState("");
  ...
  const handlePreviousPagePending = () => {
    setCurrentPagePending(prevPage => (prevPage > 1 ? prevPage - 1 : prevPage));
  };
  ...
  const handlePreviousPageCompleted = () => {
    setCurrentPageCompleted(prevPage => (prevPage > 1 ? prevPage - 1 : prevPage));
  };

  const filteredPendientes = solicitudesPendientes.filter(
    item =>
      (item.Socios_RegistroSocio.NombreSocio && item.Socios_RegistroSocio.NombreSocio.toLowerCase().includes(searchSocioPendientes.toLowerCase())) &&
      (item.EstudiantesCarta.length === 0 || item.EstudiantesCarta.some(estudiante => `${estudiante.Usuario.Nombre} ${estudiante.Usuario.Apellido1} ${estudiante.Usuario.Apellido2}`.toLowerCase().includes(searchEstudiantePendientes.toLowerCase())))
  );

  const filteredCompletadas = solicitudesCompletadas.filter(
    item =>
      (item.Socios_RegistroSocio.NombreSocio && item.Socios_RegistroSocio.NombreSocio.toLowerCase().includes(searchSocioCompletadas.toLowerCase())) &&
      (item.EstudiantesCarta.some(estudiante => `${estudiante.Usuario.Nombre} ${estudiante.Usuario.Apellido1} ${estudiante.Usuario.Apellido2}`.toLowerCase().includes(searchEstudianteCompletadas.toLowerCase()))) &&
      (item.NombreCarta && item.NombreCarta.toLowerCase().includes(searchCartaCompletadas.toLowerCase()))
  );
          ...
          {filteredPendientes.length > 0 ? (
            <div className="table-container-sociocomu">
              ...
              <div className="pagination-sociocomu">
                <button onClick={handlePreviousPagePending} disabled={currentPagePending === 1}>
                  Anterior
                </button>
                ...
                <tbody className="tbody-sociocomu">
                  {filteredCompletadas.map((solicitud, index) => (
                    <tr key={index}>
                      ...
                      <td>{solicitud.NombreCarta}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="pagination-sociocomu">
                <button onClick={handlePreviousPageCompleted} disabled={currentPageCompleted === 1}>
                  Anterior
                </button>
