...
function CrearActuCreacionGrupos() {
    ...
    const [sedes, setSedes] = useState(["Central", "Atenas", "Guanacaste", "Pacífico", "San Carlos", "C. F. P. T. E."]);
    ...
    const filterUsuariosBySede = (sede, usuariosData = allUsuarios) => {
        ...
        if (sede === "") {
            ...
            const filteredUsuarios = usuariosData.filter(usuario =>
                usuario.Sede === sede || usuario.Sede === "Todas"
            );
    ...
    const validateForm = () => {
        const requiredFields = ["CodigoMateria", "NumeroGrupo", "Horario", "Aula", "Cuatrimestre", "Anno", "Identificacion"];
        ...
        <div className="creagrup-container">
            ...
            <form className="creagrup-formcontainer" onSubmit={handleSubmit}>
                    ...
                    >
                        ...
                        {usuarios.map((usuario) => (
                            <option key={usuario.Identificacion} value={usuario.Identificacion}>
                                {usuario.Nombre.charAt(0).toUpperCase() + usuario.Nombre.slice(1)} {usuario.Apellido1.charAt(0).toUpperCase() + usuario.Apellido1.slice(1)} {usuario.Apellido2.charAt(0).toUpperCase() + usuario.Apellido2.slice(1)} {usuario.Sede === "Todas" && "(Multisede)"}
                            </option>
                        ))}
                    </select>
                </div>
                ...
                {Object.keys(errors).length > 0 && (
                    <div className="creaediUsu-errors">
                        <ul className="creaediUsu-error-list">
                            {Object.values(errors).map((error, index) => (
                                <li key={index} className="creaediUsu-error">
                                    {error}
                                </li>
                            ))}
                        </ul>
